...
public class ProjectFindByDescriptionCommand extends AbstractCommand {
    ...
    public void execute() {
        ...
        @Nullable final Project project = getServiceLocator().getProjectEndpoint().findProjectByDescription(Objects.requireNonNull(getServiceLocator().getSession().getUserId()) , description);
        if (project == null) {
